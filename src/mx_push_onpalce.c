...
static void push_in_list(result_list *buf, result_list *node, int *arr) {
    node->next =  buf->next;
    buf->next = node;
    free(arr);
}

static void cycle_list(result_list  **buf, int **arr, char **info, t_list **l) {
...
void mx_push_onpalce(result_list **list, char **info, t_list **l) {	
	result_list *node = mx_create_result(info[0], info[2], info[1]);
